...
for i in range(num_classes_4D):
	...
	save_path = os.path.join(save_folder_4d, str(i)+'.png')
	res = cv2.normalize(final_img_4D, dst=None, alpha=0, beta=255,norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)
...
for i in range(num_classes_3D):
	...
	save_path = os.path.join(save_folder_3d, str(i)+'.png')
	res = cv2.normalize(final_img_3D, dst=None, alpha=0, beta=255,norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)
	cv2.imwrite(save_path, res)
	# plot the picture
	# plt.figure()
