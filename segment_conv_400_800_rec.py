...
def segment(begin_slice, end_slice, kernel_3D_list, kernel_4D_list_1, kernel_4D_list_2, kernel_4D_list_3, 
	...
	image_batch_previous, _, _ = features.get_3D_structure(sub_path_previous, begin_slice-65, end_slice-65)   # only for t0040
	image_batch_next, _, _ = features.get_3D_structure(sub_path_next, begin_slice+60, end_slice+60)  # only for t0060
...
# update: because we now crop raw data first, there is no need to use mask
# however, to use the good segmentation method (the production of True (1) and False (0) to achieve functionn of logical 'or')
# we just create a height x width mask full of one, using as the base of production. 
...
save_path_3D = os.path.join(current_path, 'new_large_clusters_rec', 'k-means', '3d', 'cluster_128')
save_path_4D = os.path.join(current_path, 'new_large_clusters_rec', 'k-means', '4d', 'cluster_128')
...
#slice_list = [[400,411]] (for testing)
