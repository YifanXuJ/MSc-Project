'''
This file will select one time stamp, segment all classes for slice 400-800

Author: Yan Gao, Yifan Xu
...
def segment(path_img, save_path_4D, save_path_3D, model_4D, model_3D,
	...
	# do not need to get the mask and match the coordinates of the selected subregion,
	# because we can directly use the already cropped images

	pred_to_img_4D = prediction_4D.reshape(700, 855)
	pred_to_img_3D = prediction_3D.reshape(700, 855)
	
	for c in range(3):
		# final_img_4D = np.ones((height,width), np.uint8)
		# final_img_3D = np.ones((height,width), np.uint8)

		# zero_point_4D_co = np.argwhere(prediction_4D==c)
		# for i in zero_point_4D_co:
		# 	final_img_4D[coordinate[0][i], coordinate[1][i]] = 0

		# zero_point_3D_co = np.argwhere(prediction_3D==c)
		# for j in zero_point_3D_co:
		# 	final_img_3D[coordinate[0][j], coordinate[1][j]] = 0

		# use black(0) to label the current cluster
		final_img_4D = np.where(pred_to_img_4D == c, 0, 1).astype(np.uint8)
		final_img_3D = np.where(pred_to_img_3D == c, 0, 1).astype(np.uint8)
			
		print('Saving results...')
		save_png(path_img, save_path_4D, final_img_4D, c)
		save_png(path_img, save_path_3D, final_img_3D, c)
		end = time.time()
		print(end-start)
...
#mask_centre = (705, 682)
#radius = 542
keyword = 'VA10_Pc200_Ram25_Pf' #'cropped'
...
needed_slices = sub_all_tif[652:800] 
...
mask, feature_index = features.get_rec_mask_50(sub_all_tif[0], args.size)

# create folder to save the segmentation result
save_path_3D = os.path.join(current_path, 'new_large_clusters_rec', 'gmm', '3d', 'cluster_3')
save_path_4D = os.path.join(current_path, 'new_large_clusters_rec', 'gmm', '4d', 'cluster_3')
...
np.savetxt(path_3D, point_coordinate_3D, delimiter=',')
'''
print('Will segment 401 slices')
for i in needed_slices:
    segment(i, save_path_4D, save_path_3D, model_4D_type, model_3D_type, 
